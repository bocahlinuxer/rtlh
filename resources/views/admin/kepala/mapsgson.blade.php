...
<link rel="stylesheet" href="{{asset('leaflet/leaflet.css')}}">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="{{url('adminkepala/')}}"><i class="fa fa-dashboard"></i> Dashboard</a></li>
...
<script>
  $(function () {
    ...
    $.getJSON("{{url('adminkepala/ajax/rumah')}}",function(data){
      var markers = [];
      $.each(data, function(key, value){
        if(value.status == 1)
        {
          markers.push(L.marker([parseFloat(value.latitude), parseFloat(value.longitude)], {icon: redIcon})
            .bindPopup('<a href="{{url('adminkepala/rtlh')}}/'+value.id_rtlh+'">'+value.nama+'</a>'));
